...
<body class="hold-transition skin-black sidebar-collapse">
  <div class="wrapper">
    ...
    height: 100px;
      ...
      .alarmBadgeSymptom:hover {
        cursor: pointer;
      }

      .alarmBadgeContentSymptom {
        vertical-align: center;
      }

      #alarmBadgeCountSymptom {}
        ...
        <div class="row">
          <div class="col-sm-12">
            <div class="box box-solid box-grey">
              ...
              <div class="alarmBadgeSymptom">
                <div class="alarmBadgeContentSymptom">
                  <i class="fa fa-warning"></i>
                  <span id="alarmBadgeCountSymptom"><span>
                </div>
              </div>
              <?= $_content ?>
...
<script src="https://code.highcharts.com/modules/export-data.js"></script> -->
    ...
    $(document).ready(function() {
      ...
      $(window).resize(function() {
        ...
        if (newWidth < 1280) {
          ...
          $('.content').parent().height($('.content').height() * 1)
        }
      })
    })
  </script>
  <script>
    ...
    var notifyAlarmSymptom
    var alarmSymptom
    notifyKeepSymptom()
    $('.notifyKeepSymptom').show()
    var alarmMsgSymptomM1 = ""
    var alarmMsgSymptomM2 = ""
    var countAlarmSymptomM1 = 0
    var countAlarmSymptomM2 = 0
    alarmSymptom5am1()
    alarmSymptom5am2()

    function alarm() {
      wsAlarm = new WebSocket("ws://localhost:1880/ws/alarm_5aa1")
      wsAlarm.onerror = function(error) {
        console.log('Error detected: ' + error)
      }
      wsAlarm.onopen = function() {
        console.log('alarm connect')
      }
          ...
          if (checkPayload) {
            ...
            $.each(payload, function(key, value) {
              ...
              alarmMsg += `<div class='row row-eq-height '>
                          <div class='col-sm-1 float-left text-right no-padding'>
                            <span class='code-i'>${i}.</span> 
                            </div>
                          <div class='col-sm-7 float-left text-left no-padding'>
                          ...
                          <div class='col-sm-10 col-sm-offset-2 float-left text-left no-padding text-grey action-${value.code}' style='display: none; color: #ffff00'>
    ...
    function alarmSymptom5am1() {
      wsAlarmSymptom5am1 = new WebSocket("ws://localhost:1880/ws/5am1")
      wsAlarmSymptom5am1.onerror = function(error) {
        console.log('Error detected: ' + error)
      }
      wsAlarmSymptom5am1.onopen = function() {
        console.log('wsAlarmSymptom5am1 connect')
      }
      wsAlarmSymptom5am1.onclose = function() {
        console.log('wsAlarmSymptom5am1 disconnect')
        symptom5am1()
      }
      wsAlarmSymptom5am1.onmessage = function(event) {
        var payload = $.parseJSON(event.data);
        ...
        alarmMsgSymptomM1 = ""
        countAlarmSymptomM1 = 0
        keytag = Object.keys(payload)
        $.each(keytag, function(i, val) {
          motor = val.split('_')
          if (motor[0] == "motor" && parseFloat(payload[val].temperature) > tLim) {
            countAlarmSymptomM1++
            alarmMsgSymptomM1 += `<div class='row row-eq-height '>
                    <div class='col-sm-1 float-left text-right no-padding'>
                      <span class='code-i'>${countAlarmSymptomM1}.</span> 
                    </div>
                    <div class='col-sm-7 float-left text-left no-padding'>
                      <span class='code'>Temperatur Motor ${motor[1]} pada Mesin 1 melebihi standar</span><br/>
                    </div>
                    </div>`
          }
          if (motor == "motor" && parseFloat(payload[val].vibration) > vLim) {
            countAlarmSymptomM1++
            alarmMsgSymptomM1 += `<div class='row row-eq-height '>
                    <div class='col-sm-1 float-left text-right no-padding'>
                      <span class='code-i'>${countAlarmSymptomM1}.</span> 
                    ...
                    <div class='col-sm-7 float-left text-left no-padding'>
                      <span class='code'>Vibrasi Motor ${motor[1]} pada Mesin 1 melebihi standar</span><br/>
                    </div>
                    </div>`
          }
        })
        if (countAlarmSymptomM1 > 0) {}
      }
    }

    function alarmSymptom5am2() {
      wsAlarmSymptom5am2 = new WebSocket("ws://localhost:1880/ws/5am2")
      wsAlarmSymptom5am2.onerror = function(error) {
        console.log('Error detected: ' + error)
      }
      wsAlarmSymptom5am2.onopen = function() {
        console.log('wsSymptom5am2 connect')
      }
      wsAlarmSymptom5am2.onclose = function() {
        console.log('wsSymptom5am2 disconnect')
        symptom5am2()
      }
      wsAlarmSymptom5am2.onmessage = function(event) {
        var payload = $.parseJSON(event.data)
        temperature_alarm = payload.dies.temperature_alarm
        vibration_alarm = payload.dies.vibration_alarm

        tLim = parseFloat(temperature_alarm)
        vLim = parseFloat(vibration_alarm)

        alarmMsgSymptomM2 = ""
        countAlarmSymptomM2 = 0
        j = countAlarmSymptomM1
        keytag = Object.keys(payload)
        $.each(keytag, function(i, val) {
          motor = val.split('_')
          // console.log(payload[val].temperature > tLim)
          if (motor[0] == "motor" && parseFloat(payload[val].temperature) > tLim) {
            // console.log(parseInt(payload[val].temperature) > tLim)
            countAlarmSymptomM2++
            j++
            alarmMsgSymptomM2 += `<div class='row row-eq-height '>
            <div class='col-sm-1 float-left text-right no-padding'>
              <span class='code-i'>${j}.</span> 
              </div>
            <div class='col-sm-7 float-left text-left no-padding'>
              <span class='code'>Temperatur Motor ${motor[1]} pada Mesin 2 melebihi standar</span><br/>
            </div>
            </div>`
          }
          if (motor == "motor" && parseFloat(payload[val].vibration) > vLim) {
            ...
            <div class='col-sm-7 float-left text-left no-padding'>
              <span class='code'>Vibrasi Motor ${motor[1]} pada Mesin 2 melebihi standar</span><br/>
            </div>
            </div>`
          }
        })
    ...
    function notifyKeep() {
      notifyAlarm = $.notify({
        icon: 'fa fa-warning',
        title: '<span class="float-left"><b>ALARM TOYOPUC!</b></span></br>',
        message: "",
        // url: '',
        // target: '_self',
      }, {
    ...
    function notifyKeepSymptom() {
      notifyAlarmSymptom = $.notify({
        icon: 'fa fa-warning',
        title: '<span class="float-left"><b>ALARM SYMPTOM!</b></span></br>',
        message: "",
        // url: '',
        // target: '_self',
      }, {
        ...
        delay: 0,
        template: '<div data-notify="container" class="notifyKeepSymptom col-xs-11 col-sm-4 alert alert-{0}" role="alert" style="max-height:200px; @media (min-width:200px){overflow-y: scroll;}">' +
          '<button type="button" aria-hidden="true" class="close minimizeSymptom" style=" width: 30px; border: 3px solid white; color: white; opacity:.7">-</button>' +
          '<span data-notify="icon"></span> ' +
          '<span data-notify="title">{1}</span> ' +
          '<span data-notify="message">{2}</span>' +
          '<div class="progress" data-notify="progressbar">' +
    ...
    notifyKeepSymptomCheck()

    function notifyKeepSymptomCheck() {
        // console.log(countAlarmSymptomM2)
      if (countAlarmSymptomM1 > 0 || countAlarmSymptomM2 > 0) {
        if (Cookies.get('minimizeSymptom') == "true") {
          $(document).find('.minimizeSymptom').parent().hide()
          $(".alarmBadgeSymptom").show()
        } else {
          $(document).find('.minimizeSymptom').parent().show()
          $(".alarmBadgeSymptom").hide()
        }
      } else {
        $(document).find('.minimizeSymptom').parent().hide()
        $(".alarmBadgeSymptom").hide()
      }
      notifyAlarmSymptom.update('message', alarmMsgSymptomM1+alarmMsgSymptomM2)
      setTimeout(() => {
        notifyKeepSymptomCheck()
      }, 200);
    }
    ...
    $(document).on('click', '.minimizeSymptom', function(e) {
      e.preventDefault()
      $(this).parent().hide()
      $(".alarmBadgeSymptom").show()
      Cookies.set('minimizeSymptom', true)
    })
    $(".alarmBadgeSymptom").click(function(e) {
      e.preventDefault()
      $(document).find('.minimizeSymptom').parent().show()
      $(".alarmBadgeSymptom").hide()
      Cookies.set('minimizeSymptom', false)
    })
  </script>
