...
<script>
  ...
  window.onload = function() {
    ...
    $("#formSymptom").submit(function(e) {
      e.preventDefault();
      Cookies.set('line',$("#line").val())
      Cookies.set('machine',$("#machine").val())
      modul = $("#line").val() + $("#machine").val();
      date = $("#chartDate").val();
      // console.log(modul);
      get_data(modul, date)
    });
    ...
    function get_data(modul, date) {
      ...
      $.ajax({
        ...
        success: function(response) {
          // console.log(response);
          $.each(response.motor_UR, function(key, value) {
          ...
          $.each(response.motor_UL, function(key, value) {
          ...
          $.each(response.motor_LR, function(key, value) {
          ...
          $.each(response.motor_LL, function(key, value) {
    ...
    function wsock(modul) {
      ...
      wsSymptom.onerror = function(error) {
        console.log('Error detected: ' + error)
      }
      wsSymptom.onopen = function() {
        console.log('symptom connect'); /*ws.send("websocket connect");*/
      }
      ...
      wsSymptom.onmessage = function(event) {
        ...
        dies = diesNew;
        tempChart.options.data[0].dataPoints.push({
          y: parseFloat(payload.motor_UR.temperature),
          z: payload.timestamp
        });
        vibeChart.options.data[0].dataPoints.push({
          y: parseFloat(payload.motor_UR.vibration),
          z: payload.timestamp
        });
        tempChart.options.data[1].dataPoints.push({
          y: parseFloat(payload.motor_UL.temperature),
          z: payload.timestamp
        });
        vibeChart.options.data[1].dataPoints.push({
          y: parseFloat(payload.motor_UL.vibration),
          z: payload.timestamp
        });
        tempChart.options.data[2].dataPoints.push({
          y: parseFloat(payload.motor_LR.temperature),
          z: payload.timestamp
        });
        vibeChart.options.data[2].dataPoints.push({
          y: parseFloat(payload.motor_LR.vibration),
          z: payload.timestamp
        });
        tempChart.options.data[3].dataPoints.push({
          y: parseFloat(payload.motor_LL.temperature),
          z: payload.timestamp
        });
        vibeChart.options.data[3].dataPoints.push({
          y: parseFloat(payload.motor_LL.vibration),
          z: payload.timestamp
        });
        tempChart.render();
        vibeChart.render();
...
<?php } else { ?>
  <script>
    $(document).ready(function() {
    line = Cookies.get('line')
    machine = Cookies.get('machine')
      $('#line').val(line)
      $(`#machine`).val(machine)
      $(`#updateChart`).click()
    });
